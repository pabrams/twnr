...
/**
 * Retrieves the authenticated player payload attached by the {@link authenticateToken} middleware.
 * Must only be called on routes that use `authenticateToken`.
...
// REST Endpoints (auth & admin only)
 ...
 */
app.get('/api/admin/server-stats', authenticateAdmin, async (_req, res): Promise<any> => {
    try {
