...
function getRequestAuthPayload(req: Request): {
...
function resolveAuthorizedPlayerId(
    req: Request,
    res: Response,
    rawPlayerId: unknown,
): number | null {
...
async function authenticateToken(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if (!payload) {
        res.status(hadCredentials ? 403 : 401).json({
            error: hadCredentials ? 'Invalid token' : 'Authentication required',
        });
        return;
    }
...
wss.on('headers', (headers, req) => {
    ...
    headers.push(
        `Set-Cookie: ${WS_SESSION_COOKIE_NAME}=${sessionToken}; HttpOnly; Path=/; SameSite=Lax${secureFlag}`,
    );
});
...
wss.on('connection', async (ws: WebSocket, req: IncomingMessage) => {
    ...
    const sessionToken =
        wsHandshakeSessions.get(req) || crypto.randomBytes(32).toString('base64url');
        ...
        ws.on('message', async (message) => {
            ...
            if (data.type === 'move') {
                const shipRes = await pool.query(
                    'SELECT player_id FROM player_ships WHERE player_id = $1',
                    [playerId],
                );
...
app.get('/api/sector/:id', async (req, res): Promise<any> => {
    ...
    try {
        ...
        const warpsRes = await pool.query('SELECT sector_to FROM warps WHERE sector_from = $1', [
            id,
        ]);
...
app.post('/api/move', authenticateToken, async (req, res): Promise<any> => {
    ...
    broadcastTo(
        { type: 'playerMoved', playerId: pId, sector: ts, direction: 'out' },
        oldSectorClients,
    );
    broadcastTo(
        { type: 'playerMoved', playerId: pId, sector: ts, direction: 'in' },
        newSectorClients,
    );
    if (player.ws) {
        const displayWarps = warps[ts] || [];
        const playersInSector = Object.entries(players)
            .filter(([, p]) => p.sector === ts)
            .map(([id]) => Number(id));
        const sectorMsg: ServerMessage = {
            type: 'sectorDisplay',
            sector: ts,
...
app.get('/api/route/:from/:to', async (req, res): Promise<any> => {
    ...
    try {
        const sectorRes = await pool.query('SELECT id FROM sectors WHERE id IN ($1, $2)', [
            from,
            to,
        ]);
...
app.post('/api/trade', authenticateToken, async (req, res): Promise<any> => {
    ...
    try {
        ...
        if (
            ...
            await client.query('ROLLBACK');
            console.error('TRADE FAIL REASON:', portActions, action, good);
...
app.post('/api/ship/exchange', authenticateToken, async (req, res): Promise<any> => {
    ...
    try {
        ...
        if (newCargoLimit < currentCargo) {
            ...
            return res
                .status(400)
                .json({ error: 'New ship has insufficient holds for current cargo' });
        }
