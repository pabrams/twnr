...
describe('Buy holds — validation', () => {
  it('returns "Invalid quantity" for quantity 0', async () => {
    ...
    try {
      const msg = await wsRequest(ws, { type: ClientTag.BuyHolds, quantity: 0 }, ServerTag.BuyHoldsResult);
  ...
  it('returns "Invalid quantity" for negative quantity', async () => {
    const { ws } = await connectWS();
    try {
      const msg = await wsRequest(ws, { type: ClientTag.BuyHolds, quantity: -1 }, ServerTag.BuyHoldsResult);
      assert.equal(msg.type, ServerTag.Error);
      assert.equal(msg.message, 'Invalid quantity');
    } finally {
      await closeWS(ws);
  ...
  it('returns "Invalid quantity" for a float quantity', async () => {
    const { ws } = await connectWS();
    try {
      const msg = await wsRequest(ws, { type: ClientTag.BuyHolds, quantity: 0.5 }, ServerTag.BuyHoldsResult);
      assert.equal(msg.type, ServerTag.Error);
      assert.equal(msg.message, 'Invalid quantity');
    } finally {
      await closeWS(ws);
