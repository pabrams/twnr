...
describe('Ship info query', () => {
  it('returns all required fields for a new player', async () => {
    ...
    try {
      const msg = await wsRequest(ws, { type: ClientTag.ShipInfo }, ServerTag.ShipInfoResult);
      assert.equal(msg.type, ServerTag.ShipInfoResult);
...
describe('Ship info — dynamic state', () => {
  it('cargoLimit reflects holds after buying holds', async () => {
    ...
    const qty = 4;
    try {
      await wsRequest(ws, { type: ClientTag.BuyHolds, quantity: qty }, ServerTag.BuyHoldsResult);
      const msg = await wsRequest(ws, { type: ClientTag.ShipInfo }, ServerTag.ShipInfoResult);
      assert.equal(msg.type, ServerTag.ShipInfoResult);
  ...
  it('holdsAvailable reflects actual cargo (not just cargoLimit)', async () => {
    ...
    try {
      ...
      await wsRequest(ws, { type: ClientTag.PortTransaction, good: 'fuel', quantity: fuelQty, action: 'buy' }, ServerTag.PortTransactionResult);
